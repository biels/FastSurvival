    ...
    public void setArmorCD(int CD){
        ...
        taskEscutId = plugin.getServer().getScheduler().scheduleSyncDelayedTask(plugin, new Runnable(){
            public void run() {
                ...
                hpEscut = maxHpEscut;
                if (CheckArmor()){
                    getWorld().playSound(location, Sound.BLOCK_PISTON_EXTEND, 3F, 1F);
                }
...
//		}
    ...
    public enum TipusMillora {MAL, VELOCITAT_ATAC, FOC, DIST_ATAC, RESISTÈNCIA, QUIMICA, MECÀNICA, MAGNETISME, APRENENTATGE};
