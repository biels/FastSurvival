...
public class Cuboid implements Iterable<Block>, Cloneable, ConfigurationSerializable {
    ...
    public List<Block> cornersInFace(BlockFace face) {
        List<Block> res = new ArrayList<>();
        World w = this.getWorld();
        if (face == BlockFace.DOWN || face == BlockFace.WEST || face == BlockFace.NORTH)
            res.add(w.getBlockAt(this.x1, this.y1, this.z1));
        if (face == BlockFace.DOWN || face == BlockFace.WEST || face == BlockFace.SOUTH)
            res.add(w.getBlockAt(this.x1, this.y1, this.z2));
        if (face == BlockFace.UP || face == BlockFace.WEST || face == BlockFace.NORTH)
            res.add(w.getBlockAt(this.x1, this.y2, this.z1));
        if (face == BlockFace.UP || face == BlockFace.WEST || face == BlockFace.SOUTH)
            res.add(w.getBlockAt(this.x1, this.y2, this.z2));
        if (face == BlockFace.DOWN || face == BlockFace.EAST || face == BlockFace.NORTH)
            res.add(w.getBlockAt(this.x2, this.y1, this.z1));
        if (face == BlockFace.DOWN || face == BlockFace.EAST || face == BlockFace.SOUTH)
            res.add(w.getBlockAt(this.x2, this.y1, this.z2));
        if (face == BlockFace.UP || face == BlockFace.EAST || face == BlockFace.NORTH)
            res.add(w.getBlockAt(this.x2, this.y2, this.z1));
        if (face == BlockFace.UP || face == BlockFace.EAST || face == BlockFace.SOUTH)
            res.add(w.getBlockAt(this.x2, this.y2, this.z2));
        return res;
    }

    public List<Block> cornersInFaces(BlockFace face1, BlockFace face2) {
        List<Block> corners1 = cornersInFace(face1);
        List<Block> corners2 = cornersInFace(face2);
        return corners1.stream().filter(c -> corners2.contains(c)).collect(Collectors.toList());
    }

    public Block cornerInFaces(BlockFace face1, BlockFace face2, BlockFace face3) {
        List<Block> corners1 = cornersInFace(face1);
        List<Block> corners2 = cornersInFace(face2);
        List<Block> corners3 = cornersInFace(face3);
        return corners1.stream()
                .filter(c -> corners2.contains(c) && corners3.contains(c))
                .findFirst().get();
    }

    public List<Vector> lineInFaces(BlockFace face1, BlockFace face2) {
        List<Block> blocks = cornersInFaces(face1, face2);
        List<Vector> lineBetween = Utils.getLineBetween(blocks.get(0), blocks.get(1));
        return lineBetween;
    }

    public List<Vector> edgeInFaces(BlockFace face1, BlockFace face2) {
        return lineInFaces(face1, face2);
    }

    public List<List<Vector>> edgesInFaces(BlockFace f1, BlockFace f2, BlockFace f3) {
        List<BlockFace> a = Arrays.asList(f1, f2, f3);
        List<List<Vector>> r = new ArrayList<>();
        List<List<BlockFace>> combinations = getEdgeCombinations();
        for (int i = 0; i < combinations.size(); i++) {
            List<BlockFace> comb = combinations.get(i);
            if (a.containsAll(comb)) r.add(edgeInFaces(comb.get(0), comb.get(1)));
        }
        return r;
    }
    public List<List<Vector>> edges() {
        List<List<Vector>> r = new ArrayList<>();
        List<List<BlockFace>> combinations = getEdgeCombinations();
        for (int i = 0; i < combinations.size(); i++) {
            List<BlockFace> comb = combinations.get(i);
            r.add(edgeInFaces(comb.get(0), comb.get(1)));
        }
        return r;
    }

	@NotNull
	public List<List<BlockFace>> getEdgeCombinations() {
		return Arrays.asList(
				Arrays.asList(BlockFace.UP, BlockFace.SOUTH),
				Arrays.asList(BlockFace.UP, BlockFace.EAST),
				Arrays.asList(BlockFace.UP, BlockFace.NORTH),
				Arrays.asList(BlockFace.UP, BlockFace.WEST),
				Arrays.asList(BlockFace.WEST, BlockFace.SOUTH),
				Arrays.asList(BlockFace.WEST, BlockFace.DOWN),
				Arrays.asList(BlockFace.WEST, BlockFace.NORTH),
				Arrays.asList(BlockFace.SOUTH, BlockFace.EAST),
				Arrays.asList(BlockFace.SOUTH, BlockFace.DOWN),
				Arrays.asList(BlockFace.NORTH, BlockFace.DOWN),
				Arrays.asList(BlockFace.EAST, BlockFace.DOWN),
				Arrays.asList(BlockFace.NORTH, BlockFace.EAST)
		);
	}

	public List<Block> upCorners() {
        return cornersInFace(BlockFace.UP);
    }

    public List<Block> downCorners() {
        return cornersInFace(BlockFace.DOWN);
    }
    ...
    public enum CuboidDirection {
        ...
        static CuboidDirection[] all() {
            return new CuboidDirection[]{North, East, South, West, Up, Down};
        }
