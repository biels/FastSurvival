...
public class ClaySpiralPopulator extends BlockPopulator {
    ...
    public void populate(World world, Random random, Chunk chunk) {
        if (!(random.nextInt(220) <= 1)) return;
        this.world = world;
        this.random = random;
        this.chunk = chunk;
        ...
        int height = Utils.Possibilitat(2) ? Utils.NombreEntre(20, 50) : Utils.NombreEntre(12, 20);
        double radius = Utils.NombreEntre(5,10);
        int number = Utils.NombreEntre(2,4);
        double constant = Math.sqrt(height) / radius;
        for (int i = 0; i < number; i++) {
            populateSpiral( i * (360 / number) * toRadians, h -> radius, height,  0.6, false, h -> 0.001, 1000, Material.STAINED_CLAY, DyeColor.WHITE.getWoolData());
        }
